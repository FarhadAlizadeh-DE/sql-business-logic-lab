-- CORRECT: Aggregate category revenue per customer, then compute totals with a window function.
...
SELECT
  ...
  ROUND(category_revenue::numeric / NULLIF(customer_total_revenue, 0), 4) AS category_revenue_pct
FROM with_totals
